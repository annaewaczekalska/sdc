  ...
  <dialog id="alertForm" aria-labelledby="alert-dostepnosci-heading">
    <div class="dialog-content">
      <h2 id="alert-dostepnosci-heading" tabindex="-1" aria-describedby="alertInfo">
        Formularz zgłoszenia problemu z dostępnością
      </h2>
      <p>Jeśli zauważyłeś problem z dostępnością treści lub funkcji tej strony, możesz to zgłosić za pomocą poniższego formularza.</p>
      ...
      <form id="accessibilityForm" class="alert-form">
        <fieldset>
          ...
          <label>
            ...
            Anonimowe zgłoszenie problemu
          </label>
          <label>
            <input type="radio" name="typ" value="formalne">
            Formalne żądanie zapewnienia dostępności
          </label>
        </fieldset>
        ...
        <label for="email">Twój e-mail (opcjonalnie):</label>
        <input type="email" id="email" name="email" placeholder="nazwa@domena.pl">

        <div class="button-group">
          <button type="submit" id="sendForm" class="btn-submit">Wyślij zgłoszenie</button>
          <button type="button" id="closeDialog" class="btn-close">Anuluj</button>
        </div>

        <p id="alertInfo" class="small">
          Przesyłając formularz, wyrażasz zgodę na przetwarzanie danych.
        </p>
      </form>
    </div>
  </dialog>
...
<script>
  ...
  const onSubmitForm = async (event) => {
    ...
    try {
      ...
      window.alert('Wystąpił błąd podczas wysyłania. Spróbuj ponownie później.');
    } finally {
      form.inert = false;
